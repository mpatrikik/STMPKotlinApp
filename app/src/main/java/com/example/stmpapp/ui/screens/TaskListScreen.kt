...
fun TaskListScreen(viewModel: TaskViewModel) {
    ...
    val tasksFromVm by viewModel.tasks.collectAsState()
    ...
    Scaffold(
        ...
        Column(
            ...
            val filteredTasks = remember(tasksFromVm, currentFilter) {
                when (currentFilter) {
                    TaskFilter.ALL -> tasksFromVm
                    TaskFilter.COMPLETED -> tasksFromVm.filter { it.isCompleted }
                    TaskFilter.INCOMPLETE -> tasksFromVm.filter { !it.isCompleted }
                }
            }
            ...
            if (filteredTasks.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(filteredTasks.size) { index ->
                        val taskUiModel = filteredTasks[index]
                        TaskItem(
                            task = taskUiModel,
                            onToggleComplete = { viewModel.toggleCompleted(taskUiModel.id) },
                            onToggleSelect = { viewModel.toggleSelection(taskUiModel.id) }
                        )
                    }
                }
            }
            ...
            if (tasksFromVm.any { it.isSelected }) {
                Button(
